...
const navCategory = data => {
    data.forEach(category => {
        document.getElementById('nav-category').innerHTML += `<div class="mx-2"><button onclick={allDataFetch(${category.category_id})} id="${category.category_id}" class="bg-[#252525B2] py-2 px-4 text-white rounded font-semibold shadow hover:bg-[#FF1F3D] hover:text-white active:bg-[#ff6d80]">${category.category}</button></div>`
    });
};
...
const allVideo = data => {
    ...
    data.forEach(video => {
        ...
        document.getElementById('video-container').innerHTML += `
        <div>
            <img width="312px" class="rounded" src="${video.thumbnail}" alt="">
            <div class="flex items-center gap-5">
                <img width="40px" height="40px" class="rounded-full" src="${video.authors[0].profile_picture}" alt="">
                <div>
                    <h1 class="font-bold text-base">${video.title}</h1>
                    <div class="flex gap-2">
                        <p class="text-[#171717B2] text-sm">Olivia Mitchell</p>
                    ...
                    <p class="text-[#171717B2] text-sm">5.4K Views</p>
                </div>
            </div>
        </div>
        `
